#!/usr/bin/env python3
"""Cold-load, hash, and freeze the completed S4.3-PI1C checkpoint."""
...
EXPERIMENT = (
    ...
    "s43_pi1c_contact_tokens_physical_aux_seed42"
)
CHECKPOINT = EXPERIMENT / "29999"
LOG = ROOT / ".local/logs/simulation/s4_3_pi1/pi1c/train.log"
...
def write_required_pi1b_aliases() -> None:
    completion = json.loads((ARTIFACTS / "pi1b_training_manifest.json").read_text())
    manifest = json.loads((ARTIFACTS / "pi1b_checkpoint_freeze.json").read_text())
    atomic_json("pi1b_training_completion.json", completion)
    atomic_json("pi1b_checkpoint_manifest.json", manifest)


def main() -> None:
    if not CHECKPOINT.is_dir() or not LOG.is_file():
        raise SystemExit("S4_3_PI1C_FINAL_CHECKPOINT_OR_LOG_MISSING")
    ...
    gates = {
        "training_process_exited": subprocess.run(
            ["pgrep", "-f", "[t]rain_s4_3_pi1.py.*CONTACT_STATE_TOKENS_PHYSICAL_AUX"],
            capture_output=True,
        ).returncode
        != 0,
        ...
        "lambda_constant_in_log": all(
            abs(float(row["lambda_phys"]) - expected_lambda) <= 5e-5 for row in steps
        ),
        "calibration_frozen_and_unchanged": (
            calibration["status"] == "PASS"
            and calibration["lambda_phys"] == expected_lambda
            and sha256_file(calibration_path) == frozen["calibration"]["artifact_sha256"]
        ),
        "pi1b_freeze_unchanged": sha256_file(b1_freeze_path)
        == frozen["pi1b_checkpoint_freeze_sha256"],
    ...
    completion = {
        "schema": "tactile3d-unit.s4-3-pi1c-training-completion.v1",
        "status": status,
    ...
    manifest = {
        "schema": "tactile3d-unit.s4-3-pi1c-checkpoint-manifest.v1",
        "status": status,
        "frozen": status == "PASS",
        "model_identity": "B2 CONTACT_STATE_TOKENS_PHYSICAL_AUX",
        "seed": 42,
        "lambda_phys": expected_lambda,
        "optimizer_steps": 30_000,
        "checkpoint": symbolic(CHECKPOINT),
        ...
        "file_manifest": files,
        "training_completion": "$REPO_ROOT/.local/artifacts/simulation/s4_3_pi1/pi1c_training_completion.json",
        "pi1d_evaluation_performed": False,
        "gates": completion["gates"],
    }
    atomic_json("pi1c_training_completion.json", completion)
    manifest["training_completion_sha256"] = sha256_file(ARTIFACTS / "pi1c_training_completion.json")
    atomic_json("pi1c_checkpoint_manifest.json", manifest)
    atomic_json("pi1c_lambda_calibration.json", calibration)
    write_required_pi1b_aliases()
    print(
        json.dumps(
            {
                "status": status,
    ...
    if status != "PASS":
        ...
        raise SystemExit("S4_3_PI1C_COMPLETION_AUDIT_FAIL: " + ",".join(failed))
