...
def scientific_decision(
    ...
    p3_task_deltas = [value["delta"] for value in comparisons["P3-P0"]["per_task"].values()]
    mixed = min(p3_task_deltas) <= -0.05 and max(p3_task_deltas) >= 0.05
    if p3p2 == "MATERIAL_HURT" or p3p0 == "MATERIAL_HURT":
        negative_tasks = sum(value <= -0.05 for value in p3_task_deltas)
        if negative_tasks >= 2:
            return "S4_3_2_TACTILE_UNIT_HURTS_ACT"
    if mixed:
        return "S4_3_2_MIXED_TASK_DEPENDENT_RESULT"
    if p3p2 == p3p0 == "MATERIAL_IMPROVEMENT" and min(p3_task_deltas) > -0.05:
...
def human_acceptance() -> str:
    ...
    lines.extend(
        f"| {item} | PASS | `{artifact}` | `{cmd}` | {question} |"
...
def main() -> None:
    ...
    acceptance.write_text(human_acceptance(), encoding="utf-8")
    print(
        json.dumps(
