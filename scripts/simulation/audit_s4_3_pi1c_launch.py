#!/usr/bin/env python3
"""Audit the stable persistent launch of the frozen PI1C run."""
...
ARTIFACT = ARTIFACTS / "pi1c_launch.json"
FROZEN_CONFIG = ROOT / "configs/simulation/s4_3_pi1c_frozen.json"
EXPECTED_MODE = "CONTACT_STATE_TOKENS_PHYSICAL_AUX"
EXPECTED_EXP = "s43_pi1c_contact_tokens_physical_aux_seed42"
...
def gpu_snapshot(pid: int | None, physical_gpus: list[int]) -> bool:
    ...
    return (
        pid is not None
        and all(gpu in uuids for gpu in physical_gpus)
        and all((pid, uuids[gpu]) in processes for gpu in physical_gpus)
    )


def s4_2_unchanged() -> tuple[bool, bool]:
    ...
    return checkpoints == frozen["checkpoints"], configs == frozen["tracked_configs"]


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--gpu", required=True)
    parser.add_argument("--session", default="s43_pi1c_pinch")
    ...
    calibration_path = ARTIFACTS / "lambda_phys_calibration.json"
    calibration = json.loads(calibration_path.read_text())
    b1_freeze_path = ARTIFACTS / "pi1b_checkpoint_freeze.json"
    expected_lambda = float(frozen["lambda_phys"])
    ...
    if pid is not None:
        try:
            ...
            pid = None
    selected_gpus_active = gpu_snapshot(pid, physical_gpus)
    ...
    checkpoint_unchanged, configs_unchanged = s4_2_unchanged()
    ...
    gates = {
        ...
        "correct_mode": f"--mode {EXPECTED_MODE}" in command and frozen["mode"] == EXPECTED_MODE,
        ...
        "seed42": frozen["seed"] == 42 and "seed=42" in (ROOT / "scripts/simulation/train_s4_3_pi1.py").read_text(),
        "global_batch32": frozen["global_batch_size"] == 32 and "local_batch_size: 32" in text,
        "lambda_calibration_pass": calibration["status"] == "PASS",
        "lambda_artifact_frozen": sha256_file(calibration_path) == frozen["calibration"]["artifact_sha256"],
        "lambda_command_exact": f"--lambda-phys {repr(expected_lambda)}" in command,
        "lambda_metric_exact_at_log_precision": abs(float(last.get("lambda_phys", math.nan)) - expected_lambda) <= 5e-5,
        "lambda_not_recalculated": frozen["calibration"]["recalculate_during_training"] is False,
        "pi1b_freeze_unchanged": sha256_file(b1_freeze_path) == frozen["pi1b_checkpoint_freeze_sha256"],
        ...
        "contact_adapter_gradient_nonzero": float(last.get("contact_adapter_grad_norm", 0.0)) > 0,
        "physical_auxiliary_gradient_nonzero": float(last.get("physical_auxiliary_grad_norm", 0.0)) > 0,
        ...
        "s4_2_checkpoints_unchanged": checkpoint_unchanged,
        "s4_2_configs_unchanged": configs_unchanged,
        "dexjoco_revision_unchanged": run("git", "rev-parse", "HEAD", cwd=ROOT / "third_party/dexjoco").stdout.strip()
        == EXPECTED_DEXJOCO,
        "pi1d_not_run": not (ARTIFACTS / "pi1d_results.json").exists(),
    }
    payload = {
        "schema": "tactile3d-unit.s4-3-pi1c-launch.v1",
        "stage": "PI1C",
        ...
        json.dumps(
            {
                ...
                for key in (
                    ...
                    "steady_timing_samples",
                    "median_steady_state_seconds_per_step",
                    "eta_hours",
                )
            },
            sort_keys=True,
    ...
    if payload["status"] != "PASS":
        ...
        raise SystemExit("S4_3_PI1C_STARTUP_GATE_FAIL: " + ",".join(failed))
