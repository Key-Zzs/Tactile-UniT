#!/usr/bin/env bash
# Atomically select an allowed Track A GPU and hold its shared lock for "$@".
...
# Required physical audit immediately before atomic selection.  Memory alone
# is not treated as evidence of availability; the process table is included.
nvidia-smi -i 2
nvidia-smi -i 3
nvidia-smi -i 1
nvidia-smi \
