...
The audit itself initially recorded `NATIVE_SANITY_NOT_RUN_RESOURCE_LIMIT`, because its bounded task did not authorize a multi-gigabyte download and static evidence had already disproved direct semantic compatibility. The explicitly authorized, temporally corrected BVA target build subsequently downloaded the revision-pinned 5.49 GB tokenizer, verified all four published file hashes, loaded it with zero missing or unexpected keys, and executed the frozen Vision transition path for all 38,465 valid TRAIN anchors on one GPU. The canonical simulator transition is `t→t+27` at 50 Hz (0.54 s); the frozen 30 Hz policy dataset represents it by the nearest native `t→t+16` frame (0.5333 s, 0.0067 s error, no RGB interpolation). Outputs were finite `[8,32]` tensors after the frozen VA bridge. This upgrades the narrow native result to `PASS_VISION_TRANSITION_ONLY_DURING_BVA_TARGET_BUILD`; it does not test the embodiment-specific Action path and does not change the direct-compatibility failure.
