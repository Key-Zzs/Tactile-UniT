...
def test_c6_router_has_exactly_the_canonical_routes(action, contact, vision, expected):
    assert (
        route_c6_availability(
            action_available=action, contact_context_available=contact, vision_available=vision
        )
        is expected
    )
...
def test_c6_protocol_is_freeze_only_and_manifest_has_no_local_paths():
    ...
    assert all(
        component["runtime_routable"] is False
        for component in manifest["components"]
        if component["name"] in {"F_VA", "C5 causal visual"}
    )
...
def test_c6_final_evaluation_is_frozen_and_has_only_allowed_warnings():
    ...
    assert value["decision"] in {
        ...
        "C6_M3_ESTABLISHED",
        "C6_M3_NOT_ESTABLISHED_INTEGRITY_FAIL",
    }
